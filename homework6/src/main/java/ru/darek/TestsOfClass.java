...
public class TestsOfClass {
    ...
    private static String CheckSuite2(Class c) {
        for (Method m : c.getDeclaredMethods()) {
            if (! m.isAnnotationPresent(Disabled.class)) {
                if (m.isAnnotationPresent(BeforeSuite.class)) {
                    if (lists[11].size() > 0) return "Количество аннотации BeforeSuite > 1";
                    lists[11].add(m);
                }
                if (m.isAnnotationPresent(AfterSuite.class)) {
                    if (lists[0].size() > 0) return "Количество аннотации AfterSuite > 1";
                    lists[0].add(m);
                }
